---
name: "source-command-process-claude-issues"
description: "One tick of the issue-fixing loop — clean up merged PRs, pick the oldest open issue labelled `Codex`, fix it in a worktree, open a PR."
---

# source-command-process-claude-issues

Use this skill when the user asks to run the migrated source command `process-claude-issues`.

## Command Template

# Process issues labelled `Codex`
...
List candidates — open issues with the `Codex` label, excluding any already in flight (`Codex-working`) or known to be broken (`Codex-blocked`):

```bash
gh issue list --repo vdulnev/JRR_Kmp_native_ui \
  --state open --label Codex \
...
- is labelled `Codex-working` (in-flight lock, set by an earlier tick), or
- is labelled `Codex-blocked` (a previous tick gave up; needs a human to look).
...
- Add the `Codex-working` label so a parallel tick won't pick the same issue:
  `gh issue edit <N> --repo vdulnev/JRR_Kmp_native_ui --add-label Codex-working` (create the label first if it doesn't exist).
...
- Remove the `Codex-working` label (the open PR is now the lock):
  `gh issue edit <N> --repo vdulnev/JRR_Kmp_native_ui --remove-label Codex-working`
...
- **Don't touch issues without the `Codex` label.** Even if you spot something fixable.
- If anything goes wrong mid-tick (build failure you can't fix, ambiguous issue), comment on the issue explaining what blocked you, remove the `Codex-working` label, and stop. Don't open a half-baked PR.
